...
class Scan(Resource):
    ...
    def get(self):
        ...
        # Get Scan data
        scans = db.session.query(DB_objects.Scan).filter(DB_objects.Scan.room_id == args.room_id).all()
        ...
        for scan in scans:
            ...
            user = db.session.query(DB_objects.User).filter(DB_objects.User.id == scans[0].user_id).first()
            scans_data.append({
